...
class LLMS_WC_Relationship_Display {
	...
	public function product_meta_end() {
		...
		$ids = llms_get_llms_products_by_wc_product_id( get_the_ID() );

		if ( llms_wc_is_product_variable( get_the_ID() ) ) {
			$product = wc_get_product( get_the_ID() );
			foreach ( $product->get_available_variations() as $variation ) {
				$ids = array_merge( $ids, llms_get_llms_products_by_wc_product_id( $variation['variation_id'] ) );
			}
		}
